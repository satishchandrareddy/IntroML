...
class NeuralNetwork(NeuralNetwork_Base.NeuralNetwork_Base):
    ...
    def back_propagate(self,X,Y):
        ...
        for layer in range(self.nlayer-1,-1,-1):
            ...
            if layer > 0:
                self.info[layer]["param_der"]["W"] = np.dot(grad_Z_L,self.get_A(layer-1).T)+2*self.info[layer]["lambda"]*self.get_param(layer,"param","W")
                grad_A_L = np.dot(self.get_param(layer,"param","W").T,grad_Z_L)
            else:
                self.info[layer]["param_der"]["W"] = np.dot(grad_Z_L,X.T)+2*self.info[layer]["lambda"]*self.get_param(layer,"param","W")
