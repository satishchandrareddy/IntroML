#unittest_forwardbackprop_elu.py
...
class Test_functions(unittest.TestCase):
    ...
    # elu case
    def test_NeuralNetwork_binary_elu(self):
        # (1) create input/output training data X and Y
        nfeature = 2
        m = 1000
        X = np.random.rand(nfeature,m)
        Y = np.round(np.random.rand(1,m))
        # (2) define neural network - use elu in first layer
        model = NeuralNetwork.NeuralNetwork(nfeature)
        model.add_layer(5,"elu")
        model.add_layer(3,"tanh")
        model.add_layer(1,"sigmoid")
        # (3) compile
        optimizer = None
        ...
        eps = 1e-5
        error = model.test_derivative(X,Y,eps)
        print("forwardbackprop: 3 layer NeuralNetwork ELU Error: {}".format(error))
        # (5) assert
        self.assertLessEqual(error,1e-7)
